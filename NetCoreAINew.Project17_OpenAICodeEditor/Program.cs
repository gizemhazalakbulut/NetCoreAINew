...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        StringBuilder userCode = new(); // Kullanıcının girdiği kodu saklamak için StringBuilder kullanılıyor

        string? line; // Kullanıcının girdiği her satırı saklamak için geçici bir değişken
        while ((line = Console.ReadLine()) != null && line.Trim() != "END") // Kullanıcı 'END' yazana kadar satırları al
        {
            userCode.AppendLine(line); // Satırı StringBuilder'a ekle
        }

        string prompt = choice switch // Kullanıcının seçimine göre prompt oluştur
        {
